...
- [ ] **实时文件监听** — 基于 chokidar 监听工作目录变更，自动刷新文件树并触发增量同步
- [ ] **定时自动同步** — 支持用户自定义同步间隔，后台定时执行同步任务
- [ ] **同步规则过滤** — 为同步映射配置 glob 忽略规则（如 `*.log`、`node_modules/`），跳过无需同步的内容
- [ ] **双向同步** — 当前仅支持单向复制，可扩展为双向同步并自动检测冲突
- [ ] **Git 分支管理** — 可视化切换、创建、合并、删除分支
- [ ] **Git Diff 可视化** — 集成 Monaco Diff Editor，对比文件修改前后的差异
- [ ] **Git Stash 暂存** — 支持暂存当前变更并在需要时恢复
- [ ] **远程仓库管理** — 在 GUI 中添加、修改、删除远程仓库地址
- [ ] **终端集成** — 内嵌终端面板（基于 xterm.js），直接在应用内执行命令
- [ ] **文件搜索** — 全局文件名搜索与文件内容全文检索
- [ ] **多主题支持** — 提供亮色 / 暗色主题切换，支持自定义配色方案
- [ ] **多语言国际化** — 通过 i18n 支持中文 / 英文等多语言界面
- [ ] **插件系统** — 开放扩展接口，允许第三方插件接入编辑器与同步流程
- [ ] **跨平台支持** — 适配 macOS 和 Linux 的构建与打包流程
- [ ] **自动更新** — 集成 electron-updater，应用启动时检测并提示新版本
